...
``` {.default filename="Output"}
total 8
drwxrwxr-x 4 csuser csuser 4096 Mar  4 16:31 data
drwxrwxr-x 7 csuser csuser 4096 Feb 27 10:34 databases
```
...
For now, let's have a look in `raw_reads`. We can do this without changing directories using the `ls` command followed by the name of the directory.
...
``` {.default filename="Output"}
SRR6820491/  SRR6820491_1.fastq  SRR6820491_2.fastq  SRR6820516/  SRR6820516_1.fastq  SRR6820516_2.fastq
```
...
<!-- ![A file hierarchy tree](../fig/blank_instance_file_tree.png){:width="800px"} -->

![A file hierarchy tree.](/images/blank_instance_file_tree.png){width="800px" fig-alt="A file hierarchy tree."}

First we moved from our home directory at `csuser` into the `cs_course` directory, which is one level down. From there we opened up the `data` directory, which contain the subdirectory - `raw_reads`. We had a peek inside this directory and found it contained multiple directories and files.
...
For example, if we now try to list the files in `raw_reads` with names starting with `SRR` by using tab complete:

``` {.bash filename="Code"}
ls SRR<tab>
```

The shell auto-completes your command to `SRR6820`, because all file names in the directory begin with this prefix. When you hit <kbd>tab</kbd>again, the shell will list the possible choices.
...
Use the commands we've learned so far to navigate to the `raw_reads` directory from our home:
