...
Navigate to your home directory. From there, list the contents of the `raw_reads` directory.
...
Using the file system diagram below, if `pwd` displays `/home/csuser/cs_course/data/`, what will `ls ../databases` display?

Can you explain why?

<!-- ![Blank instance file tree.](../fig/blank_instance_file_tree_with_hidden.png){:width="300px"} -->

![Blank instance file tree.](/images/blank_instance_file_tree_with_hidden.png){width="800px" fig-alt="Blank instance file tree."}

::: {.callout-caution collapse="true"}
## Solution

``` {.default filename="Output"}
chocophlandb  humann  sortmerna_databases  uniref  utility_mapping
```

The command `ls ..` moves us up a folder level before we list the contents of `databases`.
:::
::::
...
Navigate to the `raw_reads`` directory:
