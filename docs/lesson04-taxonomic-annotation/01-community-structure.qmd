...
In this tutorial we shall be following the second of the approaches detailed above. In the [previous lesson](/docs/lesson03-qc-pre-processing/03-rrna-filtering.qmd) we filtered our samples to separate out rRNA and mRNA sequences, however for our taxonomic analysis we shall be using our unfiltered data.
...
(For more on different methodologies see the [introduction to lesson 3](/docs/lesson03-qc-pre-processing/01-introduction-meta.qmd)).
...
We will run MetaPhlAn on trimmed paired-end reads for each sample. MetaPhlAn4 does not actually use paired-end information during alignment but will instead treat each read independently when mapping against its marker gene database. However, by using both the forward (R1) and reverse (R2) trimmed read files for each sample we increase the total number of reads available for classification, improving both species detection sensitivity and the stability of relative abundance estimates. This means we will need to concatenate the R1 and R2 files into one file because MetaPhlAn needs a single input file.
...
Concatenate our R1 and R2 trimmed reads into one file:

``` {.bash filename="Code"}
cat data/trimmed_reads/SRR6820491_1.trim.fastq data/trimmed_reads/SRR6820491_2.trim.fastq > data/trimmed_reads/SRR6820491_merged.fastq
```
::: callout-tip
`cat file1 file2` concatenates (joins) the contents of two files in sequence, printing `file1` first, then `file2` immediately after.
`> output_file` redirects that combined output into a new file instead of printing it to the screen.
:::
...
::: callout-important
## Running MetaPhlAn a second time
Before running MetaPhlAn a second time, you will need to delete all the outputs from the first run with: `rm -r metaphlan
:::
...
`data/trimmed_reads/SRR6820491_merged.fastq` specifies our input file containing our trimmed reads. We use this rather than our sorted mRNA from sortmeRNA as MetaPhlan identifies species based on a wide range of marker genes, some of which may be filtered out by sortmeRNA (such as species-specific ribosomal genes).
...
`--index mpa_vJun23_CHOCOPhlAnSGB_202307` specifies which version of the chocophlan database we want to use.
...
#/home/csuser/.local/bin/metaphlan data/trimmed_reads/SRR6820491_merged.fastq --input_type fastq --bowtie2db databases/chocophlandb/ --index mpa_vJun23_CHOCOPhlAnSGB_202307 --bowtie2out analysis/metaphlan/metaphlan_bowtie2.bz2 --nproc 8 --add_viruses --tax_lev s --min_cu_len 2000 --read_min_len 70 --stat_q 0.1 --min_ab 0.1 --output_file analysis/metaphlan/metaphlan_output.txt
#16372080 reads processed
#SampleID       Metaphlan_Analysis
#clade_name     NCBI_tax_id     relative_abundance      additional_species
s__Coprothermobacter_proteolyticus      35786   78.0571
s__Acetivibrio_thermocellus     1515    15.07538
s__Tepidanaerobacter_sp_GT38    2722793 6.32141
s__Methanothermobacter_defluvii 49339   0.53283
s__Escherichia_coli     562     0.01328
UNCLASSIFIED    -1      0.0
```
...
2.  We only have species-level access in our run. Changing the `--tax_lev` flag would allow us to identify a broader set of taxa (e.g., genus, family, kingdom).

3.  In our sample we identified:\
    *Coprothermobacter proteolyticus, Acetivibrio thermocellus, Tepidanaerobacter sp. GT38, Methanothermobacter defluvii,* and *Escherichia coli*.

4.  Trick question! Although you can manually verify each taxon, you would need to adjust the `--tax_lev` flag to show the kingdom (`k__`) level to formally confirm whether only bacteria are present. With only the species (`s__`) level shown we cannot see if there's fungal or archaeal reads that have been identified but only resolved down to a higher taxonomic level (for example phyla or family)
:::
::::

Whilst it is possible to interpret information from this table alone, it is far easier to try to do so from visualised data. In the next episode we shall be using Krona to generate visualisations of Metaphlan's output.
