...
``` bash
 ...
 --bowtie2db data/chocophlandb/ \
 --bowtie2out analysis/metaphlan/metaphlan_bowtie2.bz2 \
 --nproc 8--add_viruses--tax_lev 's' \
 --min_cu_len 2000--read_min_len 70 \
 --stat_q 0.1--output_file analysis/metaphlan/metaphlan_output.txt \
 --min_ab 0.1&> analysis/metaphlan/log.txt &
```

This should take about 30 minutes to run. It takes a while as each sequence is compared to the full database one by one! Our test data here only consists of one sample, but in your research you may be working on many many more. As such a "real" Metaphlan run may take much longer.
