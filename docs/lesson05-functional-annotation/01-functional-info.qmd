...
::: callout-note
Usually we would include `--nucleotide-database /home/ubuntu/cs_course/data/chocophlandb/` to specify the location of the database for the nucleotide search step, but due to space limitations on our instances we are instead using `--bypass-nucleotide-search`. This is because Humann and Metaphlan both use different versions of the Chocophlan database, each version being around 80-100GB in size.

The UniRef databases can also be extremely large, with over 100GB being standard. Our instances are not quite large enough to accomodate all the databases we would need for this full pipeline. This is because for costing purposes only we're working on slightly smaller instances for this course. It is entirely possible to expand the disk space of an AWS instance, and during your own analyses we recommend about 1TB of storage (or more if you are processing more than \~20 samples).
:::
